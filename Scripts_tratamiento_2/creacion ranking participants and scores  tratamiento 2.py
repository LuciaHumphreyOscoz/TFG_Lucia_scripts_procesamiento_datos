...
# Asignar time limit según par o impar
df_scores["time_limit"] = df_scores["custom_id"].apply(lambda x: 15 if x % 2 == 0 else 18)
...
csv_path_downloads = os.path.join(downloads_path, "resultscompetitivetask_tratamiento_2.csv")
csv_path_desktop = os.path.join(desktop_folder, "resultscompetitivetask_tratamiento_2.csv")
...
pdf.cell(0, 10, "Ranking - Tarea Competitiva Tratamiento 2", ln=True, align='C')
pdf.ln(10)
...
pdf.cell(50, 10, "Rondas Jugadas", 1, 0, 'C', True)
pdf.cell(30, 10, "Time Limit", 1, 1, 'C', True)
...
for _, row in df_scores.iterrows():
    ...
    pdf.cell(50, 10, str(int(row["total_rounds_played"])), 1, 0, 'C')
    pdf.cell(30, 10, str(int(row["time_limit"])), 1, 1, 'C')

# Guardar PDF
pdf_path_downloads = os.path.join(downloads_path, "ranking_tarea_competitiva_tratamiento_2.pdf")
pdf_path_desktop = os.path.join(desktop_folder, "ranking_tarea_competitiva_tratamiento_2.pdf")
